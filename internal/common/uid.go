...
// its structure contain 62 bits
// localID: 32 bits (max 2^32 -1)
// objectType: 10 bits
// shardID: 18 bits
...
func (uid *UID) UnmarshalJSON(data []byte) error {
	decoded, err := FromBase58(strings.Replace(string(data), "\"", "", -1))

	if err != nil {
		return err
	}

	uid.localID = decoded.localID
	uid.objectType = decoded.objectType
	uid.shardID = decoded.shardID
